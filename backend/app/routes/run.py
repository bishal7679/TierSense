...
async def run_tiering(
    ...
    for ent in suggestions.get("analysis", []):
        # Normalize path
        raw_path = strip_pref(ent["path"])
        # Map to container file path
        container_path = raw_path if os.path.exists(raw_path) else f"/host-root{raw_path}"
        # Skip if still nonexistent or a directory
        if not os.path.exists(container_path) or os.path.isdir(container_path):
            continue
        # Update path and add to analysis
        ent["path"] = container_path
        analysis.append(ent)
