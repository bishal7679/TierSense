...
def perform_daily_reset():
    ...
    try:
        ...
        # Step 2: Clean up ALL NDJSON logs to force fresh start
        removed_ndjson = []
        for filename in os.listdir(LOG_DIR):
            if filename.endswith(".ndjson") and "tiersense-processed" in filename:
                ...
                try:
                    os.remove(full_path)
                    removed_ndjson.append(filename)
        ...
        # Step 4: Clean up all old heatmap files
            ...
            for f in os.listdir(heatmap_dir):
                if f.startswith("access_heatmap_") and f.endswith(".png"):
                    try:
                        ...
                        removed_heatmaps.append(f)
                        logger.info(f"Removed old heatmap during reset: {f}")
                    except Exception as e:
                        logger.warning(f"Failed to remove heatmap {f}: {e}")

        # Step 5: Restart Filebeat to create fresh log file for today
        try:
            with open(os.path.join(LOG_DIR, 'filebeat.log'), 'a') as log_file:
                subprocess.Popen(
        ...
        logger.info(
            f"Daily reset completed for {today} - removed {len(removed_ndjson)} NDJSON logs and {len(removed_heatmaps)} heatmaps"
        )
    except Exception as e:
